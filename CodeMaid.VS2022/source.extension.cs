namespace SteveCadwallader.CodeJanitor
{
    internal static class Vsix
    {
        public const string Id = "b1b6d05b-97f7-426d-9d6f-fdf8c7662ab2";
        public const string Name = "CodeJanitor";
        public const string Description = "CodeJanitor (a fork of CodeJanitor) is an open source Visual Studio extension to cleanup and simplify our C#, C++, F#, VB, PHP, PowerShell, R, JSON, XAML, XML, ASP, HTML, CSS, LESS, SCSS, JavaScript and TypeScript coding.";
        public const string Language = "en-US";
        public const string Version = "12.0";
        public const string Author = "Steve Cadwallader";
        public const string Tags = "build, code, c#, beautify, cleanup, cleaning, digging, reorganizing, formatting";
    }
}